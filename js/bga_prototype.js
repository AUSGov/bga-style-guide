...
$(document).ready(function () {
    ...
    if ($('#cool-prototype').length) {
        ...
        // Clear sessionStorage on 'start' button click
        ...
        $('.question.dynamic-trigger .radio-button').on('click', function () {
            $(this).parents('.question').next('.question').removeClass('d-none');
        });

        // Basic page validation to check all questions have an answer
        $('.next').on('click', function (e) {
            e.preventDefault();
            var destination = ($(this).attr('href'));

            if ($('.question').length) { 
                var total_questions = $('.radios:visible').length;
                var total_answered = 0; 

                $( '.radios:visible' ).each(function(){
                    var selected = 0;
                    
                    $(this).find('.radio-button input').each(function () {
                        if ( $(this).is(':checked') ) {
                            selected ++;
                        }
                    });
                ...
                if ( total_questions == total_answered ) {
        ...
        var get_answers = function () {
            ...
            $('.radios .radio-button input').each(function () {
                var id = $(this).attr('id');
                if (answers.includes(id)) {
                    $(this).prop('checked', true);
                    $(this).parents('.question').removeClass('d-none');
                } else {
                    $(this).prop('checked', false);
                };
            });
        ...
        $('.radios .radio-button input').on('click', function(){
            $(this).parents('.radios').removeClass('error');
            $(this).parents('.radios').next('.error-message').addClass('d-none');
        });
